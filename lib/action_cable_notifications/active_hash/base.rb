  ...
  class Base
    ...
    class << self
      ...
      def scoped_collection ( scope = :all )
        aux_scope = scope.deep_dup
        aux_scope = aux_scope.to_a if aux_scope.is_a? Hash
        Array(aux_scope).inject(self) do |o, a|
          o.try(*a)
        end
      end
      ...
      def select( fields=nil )
        if fields.present?
          self.all.map{|v| v.attributes.slice(*Array(fields))}
        else
          self.all
        end
      end
